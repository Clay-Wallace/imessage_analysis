...
def combine_data(recent_messages, contacts):
    ...
    # Optional: build a lookup dict for O(1) matches
    number_to_contact = {}
    for c in contacts:
        num_clean = c.get("NUMBERCLEAN")
        if not num_clean:
            continue
        # If multiple contacts share a number, last one wins
    ...
    for message in recent_messages:
        ...
        # In this AppleScript-based data, "NAME" is the full contact name
        full_name = contact.get("NAME", "")
        # You can either store the full name or split it if you like
        message["name"] = full_name

        # Example optional split into first/last:
